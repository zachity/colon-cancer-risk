...
def main():
    ...
    ap.add_argument("--xpt", required=True, help="Path to LLCP2022.XPT")
    ...
    print(f"Loading BRFSS XPT via pyreadstat: {args.xpt}")
    df, meta = pyreadstat.read_xport(args.xpt, encoding="latin1")
    ...
    unique_states = sorted(df["_STATE"].dropna().unique())
    print(f"Number of unique _STATE values in raw data: {len(unique_states)}")
    print("Sample of _STATE codes:", unique_states[:15])
    ...
    print(f"\nSaved: {out_path}")
    print(f"Number of rows in summary: {len(out)}")
